...
class App {
    constructor(element, options = {}, uuid = 0) {
        ...
        this._modules = [
            'container', 'source', 'editor', 'event', 'placeholder', 'codemirror', 'hotkeys',
            'block', 'blocks', 'sync', 'observer', 'scroll', 'autosave', 'format', 'inline', 'state', 'progress',
            'ui', 'toolbar', 'extrabar', 'addbar', 'context', 'control', 'path', 'statusbar',
            'dropdown', 'modal', 'panel', 'command',
            'image', 'embed', 'layout', 'list', 'link', 'table'
        ];
        this._plugins = [];
        this._props = {};
        this._mode = 'default';
    ...
    _init() {
        ...
        // modules
        this.container = new ContainerModule(this);
        this.source = new SourceModule(this);
        this.editor = new EditorModule(this);
        this.event = new EventModule(this);
        this.codemirror = new CodemirrorModule(this);
        this.placeholder = new PlaceholderModule(this);
        this.block = new BlockModule(this);
        this.blocks = new BlockCollectionModule(this);
        this.sync = new SyncModule(this);
        this.observer = new ObserverModule(this);
        this.scroll = new ScrollModule(this);
        this.autosave = new AutosaveModule(this);
        this.format = new FormatModule(this);
        this.inline = new InlineModule(this);
        this.state = new StateModule(this);
        this.progress = new ProgressModule(this);
        this.hotkeys = new HotkeysModule(this);

        this.image = new ImageManager(this);
        this.embed = new EmbedManager(this);
        this.layout = new LayoutManager(this);
        this.list = new ListManager(this);
        this.link = new LinkManager(this);
        this.table = new TableManager(this);

        this.ui = new UIManager(this);
        this.toolbar = new UIToolBar(this);
        this.extrabar = new UIExtraBar(this);
        this.addbar = new UIAddBar(this);
        this.context = new UIContextBar(this);
        this.control = new UIControlBar(this);
        this.path = new UIPathBar(this);
        this.statusbar = new UIStatusBar(this);
        this.dropdown = new UIDropdown(this);
        this.modal = new UIModal(this);
        this.panel = new UIPanel(this);
        this.command = new UICommandPalette(this);
...
class ConfigModule {
    ...
    _init(plugin, options) {
        ...
        const config = Redactor.extend(true, editorSettings, elementData);

        // Merge defaults and config
        for (let key in editorSettings) {
            if (options[key] && typeof options[key] === 'object' && (options[key]._replace || options[key].set)) {
                // Complete replacement of the object
                config[key] = { ...options[key] };
                delete config[key]._replace;
                delete options[key]._replace;
                delete config[key].set;
                delete options[key].set;
            } else if (options[key]) {
                // Merging objects
                config[key] = Redactor.extend(true, editorSettings[key], options[key]);
            }
        }
...
class IframeBuildStrategy extends BuildStrategy {
    ...
    _buildCustomJs() {
        ...
        return this.config.get('custom.js').filter(script => typeof script === 'object' && script.head).map(script => {
            return this.dom('<script>').attr('src', script.src).outer();
        }).join('');
    }
    ...
    _createScripts() {
        if (!this.config.is('custom.js')) return '';
        return this.config.get('custom.js').filter(script => typeof script === 'object' && !script.head).map(script => {
            const src = script.src + '?' + new Date().getTime();
            return this.dom('<script>').addClass('rx-js').attr({ src }).outer();
        }).join('');
    }
...
class EventModule {
    ...
    isOutsideEditor(e) {
        const $target = this.dom(e.target);
        const targets = ['-modal-', '-form-', '-toolbar-container-', '-source-container-', '-control-', '-context-'];
...
class UIManager {
    ...
    buildButtons(type, blockInstance) {
        const buttons = this._getButtonsFromconfig(type, blockInstance);
    ...
    _makeButtons(type, $container, buttons, customButtons, removeButtons) {
        const extendFromconfig = this._getExtendFromconfig(type);
...
class StateModule {
    ...
    load() {
        this.clear();
        if (this.app.isMode('iframe')) {
            this.editor.getEditor().on('load', this._loadInitialState.bind(this));
        } else {
...
class TableManager {
    ...
    popup(e, button) {
        ...
        if (instance.isType('table')) {
            Object.entries(items).forEach(([key, item]) => {
                if (key !== 'removetable') items[key].disable = true;
            });
        }
